...
class CrossAttention2(nn.Module):
    def __init__(self, dim1, dim2, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., pool_ratio=16, eps=1e-5):
        super().__init__()
        assert dim1 % num_heads == 0, f"dim {dim1} should be divided by num_heads {num_heads}."
        ...
        self.proj = nn.Linear(dim1, dim1)
        self.proj_drop = nn.Dropout(proj_drop)
        self.act_relu = nn.ReLU(inplace=False)
        self.eps = eps
    ...
    def forward(self, x, y, H2, W2, H1, W1):
        ...
        attn = q @ kv
        attn = attn[..., :-1] / (attn[..., -1:] + self.eps) #이건 본인들이 논문에 언급하지 않은 기술인듯 (Google에서 쓴 논문에서 참고한 듯)
        attn = self.attn_drop(attn)

        x = attn.transpose(1, 2).reshape(B1, N1, C1)
        x = self.proj(x)
        x = self.proj_drop(x)
...
@MODELS.register_module()
class APFormerHeadCity2(BaseDecodeHead):
    """
    Attention-Pooling Former
    """
    def __init__(self, feature_strides, pool_scales=(1, 2, 3, 6), **kwargs):
        super(APFormerHeadCity2, self).__init__(input_transform='multiple_select', **kwargs)
        assert len(feature_strides) == len(self.in_channels)
        assert min(feature_strides) == feature_strides[0]
        self.feature_strides = feature_strides
        ...
        self.attn_c4 = Block2(dim1=c4_in_channels, dim2=tot_channels, num_heads=num_heads[0], mlp_ratio=4,
                                drop_path=0.1, pool_ratio=8)
        self.attn_c3 = Block2(dim1=c3_in_channels, dim2=tot_channels, num_heads=num_heads[1], mlp_ratio=4,
                                drop_path=0.1, pool_ratio=4)
        self.attn_c2 = Block2(dim1=c2_in_channels, dim2=tot_channels, num_heads=num_heads[2], mlp_ratio=4,
                                drop_path=0.1, pool_ratio=2)
        self.attn_c1 = Block2(dim1=c1_in_channels, dim2=tot_channels, num_heads=num_heads[3], mlp_ratio=4,
                                drop_path=0.1, pool_ratio=1)
