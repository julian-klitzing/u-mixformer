...
class Recorder:
    ...
    def record_data_hook(self, model: nn.Module, input: Type, output: Type):
        self.data_buffer.append(input)

    def record_data_hook2(self, model: nn.Module, input: Type, output: Type):
...
def visualize(args, model, recorder, result):
    ...
    in_ch = [256, 160, 64, 32]
    # in_ch = [256, 256, 256, 256] #SegFormer 는 매 stage 마다 256
    # add feature map to wandb visualizer
    feature = recorder.data_buffer[0][0][0]  # remove the batch
    features = torch.split(feature, in_ch, dim=0)
    idx = 4
    for i in range(4):
        drawn_img = seg_visualizer.draw_featmap(
            features[i], image, channel_reduction='select_max', alpha=1)
        # drawn_img = seg_visualizer.draw_featmap(
        #     features[i], image, channel_reduction='squeeze_mean', alpha=1)
        seg_visualizer.add_image(f'Decoder {idx}', 255-np.mean(drawn_img, axis=-1,keepdims=True).repeat(3, axis=2))
        idx -= 1

    feature = recorder.data_buffer[1][0]  # remove the batch
    # feature = recorder.data_buffer[1][0][0]  # remove the batch (segformer case)
    drawn_img = seg_visualizer.draw_featmap(
            feature, image, channel_reduction='select_max', alpha=1)
    # drawn_img = seg_visualizer.draw_featmap(
    #         feature, image, channel_reduction='squeeze_mean', alpha=1)
    seg_visualizer.add_image(f'Final', 255-np.mean(drawn_img, axis=-1,keepdims=True).repeat(3, axis=2))

    # for i in range(len(recorder.data_buffer)):
    #     feature = recorder.data_buffer[i][0][0]  # remove the batch
    #     for j in range(4):
    #         drawn_img = seg_visualizer.draw_featmap(
...
def main():
    ...
    source = [
        # 'decode_head.attn_c4.mlp.fc2',
        # 'decode_head.attn_c3.mlp.fc2',
        # 'decode_head.attn_c2.mlp.fc2',
        # 'decode_head.attn_c1.mlp.fc2',
        'decode_head.linear_fuse.conv',
        'decode_head.linear_fuse.activate',
        # 'decode_head.attn_c1.attn.attn_drop'
    ...
    for name, module in model.named_modules():
        if name in source:
            if 'conv' in name:
                ...
                if count == len(source):
                    break
            elif 'activate' in name or 'attn.act':
                count += 1
                module.register_forward_hook(recorder.record_data_hook2)
                if count == len(source):
                    break
